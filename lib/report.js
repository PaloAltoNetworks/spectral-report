...
const generateHTMLReport = (messages, filename, lines) => {
  ...
  // filter messages if > 1000
  const maxRows = 1000;
  if (messages.length > 1000) {
    console.log(
      chalk.yellow("Truncating results to 1000 for optimal performance...")
    );
    let slicedMessages = [];
    const unique = messages
      .map((message) => message.code)
      .filter((value, index, self) => self.indexOf(value) === index);
    const maxMessages = Math.floor(1000 / unique.length);
    unique.map((code) => {
      const slicedGroup = messages
        .filter((message) => {
          return message.code === code;
        })
        .slice(0, maxMessages);
      slicedMessages.push(slicedGroup);
    });
    messages = slicedMessages.flat();
    jsonObj.truncated = true;
  }
  ...
  messages.map((message) => {
    ...
    // assign message type
    if (message.code.startsWith("owasp:")) {
      message.type = "OWASP";
    } else {
